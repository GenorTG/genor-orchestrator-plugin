...
function getSchemaVersion() {
    try {
        const row = getDb().prepare("SELECT COALESCE(MAX(version), 0) as v FROM _schema_version").get();
        return row?.v || 0;
    }
    catch {
        return 0;
...
function applyMigration(version, name, sql) {
    const db = getDb();
    db.exec(sql);
    db.prepare("INSERT INTO _schema_version (version, name, applied_ts) VALUES (?, ?, ?)").run(version, name, Math.floor(Date.now() / 1000));
...
function migrateFromFiles(dataDir) {
    const dir = getDataDir(dataDir);
    const db = getDb();
    ...
    if (fs.existsSync(cfgFile)) {
        try {
            ...
            for (const k of ["free_only_mode", "disabled_models", "safeguards", "theme", "auto_refresh_seconds"]) {
                if (cfg[k] !== undefined)
                    stmt.run(k, JSON.stringify(cfg[k]));
            }
            // Per-project configs
            if (cfg.projects) {
    ...
    if (fs.existsSync(projDir)) {
        const sstmt = db.prepare("INSERT OR REPLACE INTO sessions (id, project, agent, model, tags, status, task, start_ts, end_ts, duration, session_key, extra, logged_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        for (const pname of fs.readdirSync(projDir)) {
            ...
            try {
                ...
                for (const s of sessions) {
                    try {
                        sstmt.run(s.id || "", pname, s.agent || "", s.model || "", JSON.stringify(s.tags || []), s.status || "", s.task || "", s.start_ts || (s.logged_at ? new Date(s.logged_at).getTime() : null), s.end_ts || null, s.duration || "", s.session_key || "", JSON.stringify({
    ...
    // ── 3. Backlog tasks ──
    if (fs.existsSync(projDir)) {
        const bstmt = db.prepare("INSERT OR REPLACE INTO backlog_tasks (id, project, title, description, priority, status, labels, depends_on, assigned_to, session_refs, created_ts, updated_ts) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        for (const pname of fs.readdirSync(projDir)) {
            if (pname.startsWith("."))
                continue;
            const blf = path.join(projDir, pname, "BACKLOG.json");
            if (!fs.existsSync(blf))
                continue;
            try {
                ...
                for (const t of tasks) {
                    try {
                        bstmt.run(t.id || "", pname, t.title || "", t.description || "", t.priority || "p2", t.status || "todo", JSON.stringify(t.labels || []), JSON.stringify(t.depends_on || []), t.assigned_to || "", JSON.stringify(t.session_refs || []), t.created_ts || Math.floor(Date.now() / 1000), t.updated_ts || Math.floor(Date.now() / 1000));
                    }
                    catch { /* skip */ }
                }
            }
    ...
    if (fs.existsSync(mf)) {
        try {
            ...
            for (const m of modelsList) {
                try {
                    mstmt.run(m.id || "", m.name || m.id || "", m.provider || "", m.agent_ready !== false && m.status !== "removed" ? 1 : 0, m.status || "active", JSON.stringify(m));
                }
                catch { /* skip */ }
            }
        }
    ...
    if (fs.existsSync(laf)) {
        try {
            ...
            for (const a of agents) {
                try {
                    lstmt.run(a.id || a.name || "?", JSON.stringify(a));
                }
                catch { /* */ }
            }
        }
    ...
    if (fs.existsSync(lsf)) {
        try {
            ...
            for (const s of sessions) {
                try {
                    lstmt.run(s.id || s.session_id || "?", JSON.stringify(s));
                }
                catch { /* */ }
            }
    ...
    if (fs.existsSync(prf)) {
        try {
            ...
            for (const e of entries) {
                try {
                    prstmt.run(e.session_key || e.id || "", e.project || "", JSON.stringify(e.tags || []), JSON.stringify(e));
                }
                catch { /* */ }
            }
        }
...
export function getGlobalConfig(key) {
    const row = getDb().prepare("SELECT value FROM global_config WHERE key = ?").get(key);
    if (!row)
        return undefined;
    try {
...
export function getAllGlobalConfig(limit = 500) {
    const rows = getDb().prepare("SELECT key, value FROM global_config LIMIT ?").all(limit);
    const result = {};
    for (const r of rows) {
        try {
...
export function getProjectConfig(project) {
    ...
    if (row.location)
        config.location = row.location;
    return config;
}
export function getAllProjectConfigs(limit = 500) {
    const rows = getDb().prepare("SELECT project, config, location FROM project_configs LIMIT ?").all(limit);
    const result = {};
    for (const r of rows) {
        const cfg = JSON.parse(r.config || "{}");
        if (r.location)
            cfg.location = r.location;
        result[r.project] = cfg;
    }
    return result;
}
export function setProjectConfig(project, config) {
...
export function updateProjectConfig(project, updates) {
...
export function listSessions(project, limit = 200, offset = 0) {
    return getDb().prepare("SELECT * FROM sessions WHERE project = ? ORDER BY start_ts DESC LIMIT ? OFFSET ?").all(project, limit, offset);
}
export function getAllSessions(limit = 1000) {
    return getDb().prepare("SELECT * FROM sessions ORDER BY start_ts DESC LIMIT ?").all(limit);
}
...
export function addSession(session) {
    const db = getDb();
    db.prepare("INSERT OR REPLACE INTO sessions (id, project, agent, model, tags, status, task, start_ts, end_ts, duration, session_key, extra, logged_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run(session.id, session.project, session.agent || "", session.model || "", session.tags || "[]", session.status || "", session.task || "", session.start_ts || null, session.end_ts || null, session.duration || "", session.session_key || "", session.extra || "{}", session.logged_at || "");
}
...
export function countSessions(project) {
    const row = getDb().prepare("SELECT count(*) as cnt FROM sessions WHERE project = ?").get(project);
    return Number(row?.cnt) || 0;
}
// ── BACKLOG ────────────────────────────────────────────────────
export function listBacklogTasks(project, limit = 1000) {
    return getDb().prepare("SELECT * FROM backlog_tasks WHERE project = ? ORDER BY CASE priority WHEN 'p0' THEN 0 WHEN 'p1' THEN 1 WHEN 'p2' THEN 2 WHEN 'p3' THEN 3 WHEN 'high' THEN 0 WHEN 'medium' THEN 2 WHEN 'low' THEN 4 ELSE 5 END, created_ts DESC LIMIT ?").all(project, limit);
}
export function getBacklogTask(id) {
    return getDb().prepare("SELECT * FROM backlog_tasks WHERE id = ?").get(id) || null;
}
export function addBacklogTask(task) {
    const db = getDb();
    db.prepare("INSERT OR REPLACE INTO backlog_tasks (id, project, title, description, priority, status, labels, depends_on, assigned_to, session_refs, created_ts, updated_ts) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run(task.id, task.project, task.title, task.description || "", task.priority || "p2", task.status || "todo", task.labels || "[]", task.depends_on || "[]", task.assigned_to || "", task.session_refs || "[]", task.created_ts || Math.floor(Date.now() / 1000), task.updated_ts || Math.floor(Date.now() / 1000));
}
export function updateBacklogTask(id, updates) {
    const fields = [];
    const values = [];
    for (const [k, v] of Object.entries(updates)) {
        if (!BACKLOG_COLUMNS.has(k))
            continue;
        if (k === "id" || k === "project")
            continue;
        fields.push(`${k} = ?`);
...
export function deleteBacklogTasksByStatus(project, status) {
    const r = getDb().prepare("DELETE FROM backlog_tasks WHERE project = ? AND status = ?").run(project, status);
    return Number(r.changes);
}
export function countBacklogByStatus(project) {
    const rows = getDb().prepare("SELECT status, count(*) as cnt FROM backlog_tasks WHERE project = ? GROUP BY status").all(project);
    const result = {};
    for (const r of rows)
...
// ── STATE EVENTS ───────────────────────────────────────────────
export function addStateEvent(project, type, data) {
    getDb().prepare("INSERT INTO state_events (project, type, data, ts) VALUES (?, ?, ?, ?)").run(project, type, JSON.stringify(data), Math.floor(Date.now() / 1000));
}
export function getStateEvents(project, limit = 200) {
    return getDb().prepare("SELECT * FROM state_events WHERE project = ? ORDER BY ts DESC LIMIT ?").all(project, limit);
}
export function pruneStateEvents(project, cutoffTs) {
    const r = getDb().prepare("DELETE FROM state_events WHERE project = ? AND ts < ?").run(project, cutoffTs);
    return Number(r.changes);
}
// ── MODELS ────────────────────────────────────────────────────
export function listModels(filterDisabled = false, project) {
    const db = getDb();
...
export function getModel(id) {
    const row = getDb().prepare("SELECT config FROM models WHERE id = ?").get(id);
    if (!row)
        return null;
    return JSON.parse(row.config);
}
export function upsertModel(id, config) {
    getDb().prepare("INSERT OR REPLACE INTO models (id, name, provider, agent_ready, status, config) VALUES (?, ?, ?, ?, ?, ?)").run(id, config.name || config.id || id, config.provider || "", config.agent_ready !== false && config.status !== "removed" ? 1 : 0, config.status || "active", JSON.stringify(config));
}
...
export function countModels() {
    const total = Number(getDb().prepare("SELECT count(*) as cnt FROM models").get()?.cnt) || 0;
    const active = Number(getDb().prepare("SELECT count(*) as cnt FROM models WHERE agent_ready = 1 AND status != 'removed'").get()?.cnt) || 0;
    return { total, active };
}
// ── LIVE AGENTS ────────────────────────────────────────────────
export function getLiveAgents(limit = 500) {
    const rows = getDb().prepare("SELECT data FROM live_agents LIMIT ?").all(limit);
...
// ── LIVE SESSIONS ──────────────────────────────────────────────
export function getLiveSessions(limit = 500) {
    const rows = getDb().prepare("SELECT data FROM live_sessions LIMIT ?").all(limit);
    const sessions = rows.map((r) => JSON.parse(r.data));
    const metaRow = getDb().prepare("SELECT value FROM global_config WHERE key = 'live_session_meta'").get();
    let meta = {};
    if (metaRow) {
        try {
            ...
            console.error("[orchestrator-db] getLiveSessions: invalid meta JSON:", e.message);
        }
    }
    return { sessions, meta };
}
export function setLiveSessions(sessions, meta) {
    const db = getDb();
...
// ── PENDING REGISTRATIONS ─────────────────────────────────────
export function getPendingRegistrations(limit = 500) {
    const rows = getDb().prepare("SELECT data FROM pending_registrations LIMIT ?").all(limit);
    return rows.map((r) => JSON.parse(r.data));
}
export function addPendingRegistration(reg) {
    getDb().prepare("INSERT OR REPLACE INTO pending_registrations (session_key, project, tags, data) VALUES (?, ?, ?, ?)").run(reg.session_key || reg.id || "", reg.project || "", JSON.stringify(reg.tags || []), JSON.stringify(reg));
}
export function removePendingRegistration(sessionKey) {
    getDb().prepare("DELETE FROM pending_registrations WHERE session_key = ?").run(sessionKey);
}
// ── CONTROL RESULTS ────────────────────────────────────────────
export function getControlResults(limit = 500) {
    const rows = getDb().prepare("SELECT name, data, ts FROM control_results ORDER BY ts DESC LIMIT ?").all(limit);
    return rows.map((r) => ({
        name: r.name,
...
export function addControlResult(name, data) {
    getDb().prepare("INSERT INTO control_results (name, data, ts) VALUES (?, ?, ?)").run(name, JSON.stringify(data), Math.floor(Date.now() / 1000));
}
// ── LOGS ───────────────────────────────────────────────────────
export function getLogs(limit = 50, level) {
...
export function addLog(level, source, message, data) {
    getDb().prepare("INSERT INTO logs (level, source, message, data, ts) VALUES (?, ?, ?, ?, ?)").run(level, source, message, JSON.stringify(data || {}), Math.floor(Date.now() / 1000));
}
export function addVerificationRun(run) {
    const db = getDb();
...
export function updateVerificationRun(pipelineId, updates) {
    const fields = [];
    const values = [];
...
export function listVerificationRuns(project, limit = 20) {
...
export function listWorkers(project) {
    return getDb().prepare("SELECT * FROM workers WHERE project = ?").all(project);
}
export function getWorker(id) {
    return getDb().prepare("SELECT * FROM workers WHERE id = ?").get(id);
}
export function addWorker(id, name, role, sprite, model, prompt, room, project, is_pm) {
    getDb().prepare(`
...
export function countWorkers(project) {
    const row = getDb().prepare("SELECT COUNT(*) as cnt FROM workers WHERE project = ?").get(project);
    return Number(row?.cnt) || 0;
}
export function deleteWorkersByProject(project) {
...
export function listRooms(project) {
    return getDb().prepare("SELECT * FROM rooms WHERE project = ?").all(project);
}
...
export function addRoom(id, name, purpose, taskTypes, project) {
    getDb().prepare(`
...
export function listVaultDocs(project) {
    return getDb().prepare("SELECT * FROM vault_docs WHERE project = ?").all(project);
}
export function getVaultDoc(path, project) {
    return getDb().prepare("SELECT * FROM vault_docs WHERE path = ? AND project = ?").get(path, project);
}
export function addVaultDoc(path, title, content, folder, project, tags, status, links, icon) {
    const now = new Date().toISOString();
    getDb().prepare(`
    INSERT OR REPLACE INTO vault_docs (id, path, title, content, folder, project, tags, status, links, icon, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(path.replace(/[^a-zA-Z0-9]/g, "_"), path, title || path, content || "", folder || "", project || "genor-orchestrator-plugin", tags || "[]", status || "", links || "[]", icon || "📄", now);
}
export function deleteVaultDoc(id) {
    getDb().prepare("DELETE FROM vault_docs WHERE id = ?").run(id);
}
export function deleteVaultDocsByProject(project) {
...
export function listPmChat(project) {
    return getDb().prepare("SELECT * FROM pm_chat WHERE project = ? ORDER BY created_at ASC").all(project);
}
export function addPmChat(message, sender, project) {
    getDb().prepare(`
    INSERT INTO pm_chat (message, sender, project)
    VALUES (?, ?, ?)
  `).run(message, sender || "user", project || "genor-orchestrator-plugin");
}
export function clearPmChat(project) {
    getDb().prepare("DELETE FROM pm_chat WHERE project = ?").run(project);
}
// ═══════════════════════════════════════════════════════════════
//  WORKER MESSAGES — CRUD
// ═══════════════════════════════════════════════════════════════
export function addWorkerMessage(fromWorker, toWorker, type, content, taskId) {
    getDb().prepare(`
...
export function listWorkerMessages(workerId, unreadOnly) {
    let query = "SELECT * FROM worker_messages WHERE to_worker = ?";
    if (unreadOnly)
        query += " AND read_at IS NULL";
    query += " ORDER BY created_at DESC LIMIT 50";
    return getDb().prepare(query).all(workerId);
}
export function deleteWorkerMessagesByWorker(workerId) {
    getDb().prepare("DELETE FROM worker_messages WHERE from_worker = ? OR to_worker = ?").run(workerId, workerId);
}
export function deleteWorkerMessagesByProject(project) {
...
//  WORKER SESSIONS
// ═══════════════════════════════════════════════════════════════
export function deleteWorkerSessionsByWorker(workerId) {
    getDb().prepare("DELETE FROM worker_sessions WHERE worker_id = ?").run(workerId);
}
export function deleteWorkerSessionsByProject(project) {
...
// ═══════════════════════════════════════════════════════════════
export function addWorkerTaskHistory(workerId, taskId, action, details) {
    getDb().prepare(`
    INSERT INTO worker_task_history (worker_id, task_id, action, details)
    VALUES (?, ?, ?, ?)
  `).run(workerId, taskId, action, details);
}
export function listWorkerTaskHistory(workerId, limit = 10) {
    return getDb().prepare("SELECT * FROM worker_task_history WHERE worker_id = ? ORDER BY created_at DESC LIMIT ?").all(workerId, limit);
}
export function getWorkerLastActivity(workerId) {
    return getDb().prepare("SELECT created_at FROM worker_task_history WHERE worker_id = ? ORDER BY created_at DESC LIMIT 1").get(workerId);
}
export function getWorkerCurrentTask(workerId) {
    return getDb().prepare("SELECT * FROM backlog_tasks WHERE worker_id = ? AND status != 'done' LIMIT 1").get(workerId);
}
export function getStalledTasksForWorker(workerId) {
    return getDb().prepare("SELECT * FROM backlog_tasks WHERE worker_id = ? AND status IN ('in_progress', 'testing')").all(workerId);
}
export function deleteWorkerTaskHistoryByWorker(workerId) {
    getDb().prepare("DELETE FROM worker_task_history WHERE worker_id = ?").run(workerId);
}
export function deleteWorkerTaskHistoryByProject(project) {
...
// ═══════════════════════════════════════════════════════════════
export function deleteSessionsByProject(project) {
    getDb().prepare("DELETE FROM sessions WHERE project = ?").run(project);
}
// ═══════════════════════════════════════════════════════════════
//  BACKLOG — bulk operations
// ═══════════════════════════════════════════════════════════════
export function countBacklogByProject(project) {
    const row = getDb().prepare("SELECT COUNT(*) as cnt FROM backlog_tasks WHERE project = ?").get(project);
    return Number(row?.cnt) || 0;
}
export function deleteBacklogByProject(project) {
    getDb().prepare("DELETE FROM backlog_tasks WHERE project = ?").run(project);
}
// ═══════════════════════════════════════════════════════════════
//  STATE EVENTS — bulk
// ═══════════════════════════════════════════════════════════════
export function deleteStateEventsByProject(project) {
