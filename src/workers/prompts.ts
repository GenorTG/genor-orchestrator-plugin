...
import type { WorkerRow, BacklogRow } from "../db.js";

const ROLE_HEADER = (worker: Pick<WorkerRow, "name" | "role">, project: string): string => `You are **${worker.name}** — a ${worker.role} on the **${project}** project. You execute tasks autonomously using the available tools (read_file, write_file, edit_file, list_files, run_command, git_commit, report_done).
...
function classifyRole(role: string): string {
  ...
  if (r.includes("manager") || r.includes("pm") || r === "pm") return "pm";
  if (r.includes("developer") || r.includes("dev") || r.includes("engineer") && !r.includes("qa")) return "developer";
  if (r.includes("qa") || r.includes("test")) return "qa";
  if (r.includes("review") || r.includes("reviewer")) return "reviewer";
  if (r.includes("design")) return "designer";
  if (r.includes("devops") || r.includes("ops") || r.includes("sre")) return "devops";
  return "default";
}
...
export function buildRolePrompt(
  ...
  const _worker: WorkerRow = worker as any; // tolerate partial
