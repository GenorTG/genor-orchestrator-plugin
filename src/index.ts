...
Focus specifically on: ${params.topic}` : "";
    ...
    const startSpawnQueueDrainer = () => {
      ...
      const drain = async () => {
        if (!fs.existsSync(SPAWN_QUEUE_PATH)) return;
        try {
          const raw = JSON.parse(fs.readFileSync(SPAWN_QUEUE_PATH, "utf-8"));
          const entries: Array<{ sessionKey: string; project: string; task: string; model?: string; message: string }> = Array.isArray(raw) ? raw : [];
          ...
          if (entries.length > 0) {
            ...
            try { fs.unlinkSync(SPAWN_QUEUE_PATH); } catch { /* */ }
          }
        } catch (e: any) {
