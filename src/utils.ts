...
// ── Tool result helper ─────────────────────────────────────────
export function txt(data: any): { content: Array<{ type: "text"; text: string }>; details: any } {
  return {
...
export function extractTags(task: string, notes: string): string[] {
  const tags: string[] = [];
...
export function readFileContent(p: string): string | null {
