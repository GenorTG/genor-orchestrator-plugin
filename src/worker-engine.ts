...
export class WorkerEngine {
  ...
  private maxToolCallIterations = 20; // Safety limit
...
Return a summary of what you did when complete.`;
    ...
    for (let iteration = 0; iteration < this.maxToolCallIterations; iteration++) {
      ...
      for (const toolCall of toolCalls) {
        ...
        if (toolCall.function.name === "write" || toolCall.function.name === "edit" || toolCall.function.name === "apply_patch") {
          ...
          if (args.path) filesChanged.push(args.path);
          if (args.input) {
  ...
  private async execCommand(command: string, workdir?: string): Promise<string> {
  ...
  private async editFile(path: string, edits: Array<{ oldText: string; newText: string }>): Promise<string> {
    const fs = await import("node:fs/promises");
...
let _workerEngine: WorkerEngine | null = null;

export function getWorkerEngine(): WorkerEngine {
  if (!_workerEngine) {
    const config = (global as any).__SOFTWARE_HOUSE_CONFIG__ || {
      gatewayToken: null,
